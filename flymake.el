...
(defvar flymake-split-error/warning-message-pattern "[\r\n]+")
(make-variable-buffer-local 'flymake-split-error/warning-message-pattern) 
...
Returns ((LINES) RESIDUAL)."
  (when (and output (> (length output) 0))
    (let* ((lines (flymake-split-string output flymake-split-error/warning-message-pattern))
